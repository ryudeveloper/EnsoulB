        ...
        private static void OnCreate(GameObject sender,EventArgs args)
        {
                    ...
                    ObjectManager.Player.CharacterName == "Karma")
                {
                    bestAllies = GameObjects.AllyHeroes
                        .Where(t =>
                            t.Distance(ObjectManager.Player) < RyusungAIO.Common.Champion.E.Range)
                        .OrderBy(x => x.Health);
                }
...

                {
                    bestAllies = GameObjects.AllyHeroes
                        .Where(t =>
                            t.Distance(ObjectManager.Player) < RyusungAIO.Common.Champion.W.Range)
                        .OrderBy(x => x.Health);
                }
                ...
                foreach (var ally in bestAllies)
                {
                    if (aaa == ally)
                    {
                        if (ObjectManager.Player.CharacterName == "Janna" ||
                            ...
                            ObjectManager.Player.CharacterName == "Karma")
                        {
                            if (RyusungAIO.Evade.EvadeTargetManager.Menu["whitelist"][
                                        ally.CharacterName.ToLower()] &&
                                aaa.Distance(ObjectManager.Player) < RyusungAIO.Common.Champion.E.Range)
                            {
                                RyusungAIO.Common.Champion.E.CastOnUnit(aaa);
                            }
                        }
                        if (ObjectManager.Player.CharacterName == "Lux" || ObjectManager.Player.CharacterName == "Sona" ||
                            ObjectManager.Player.CharacterName == "Taric")

                        {
                            if (RyusungAIO.Evade.EvadeTargetManager.Menu["whitelist"][
                                        ally.CharacterName.ToLower()] &&
                                aaa.Distance(ObjectManager.Player) < RyusungAIO.Common.Champion.W.Range)
                            {
                                RyusungAIO.Common.Champion.W.CastOnUnit(aaa);
                            }
                        }
                    }
