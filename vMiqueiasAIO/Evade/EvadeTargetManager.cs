        ...
        private static void OnCreate(GameObject sender,EventArgs args)
        {
                    ...
                    ObjectManager.Player.CharacterName == "Karma")
                {
                    bestAllies = GameObjects.AllyHeroes
                        .Where(t =>
                            t.Distance(ObjectManager.Player) < vMiqueiasAIO.Common.Champion.E.Range)
                        .OrderBy(x => x.Health);
                }
...

                {
                    bestAllies = GameObjects.AllyHeroes
                        .Where(t =>
                            t.Distance(ObjectManager.Player) < vMiqueiasAIO.Common.Champion.W.Range)
                        .OrderBy(x => x.Health);
                }
                ...
                foreach (var ally in bestAllies)
                {
                    if (aaa == ally)
                    {
                        if (ObjectManager.Player.CharacterName == "Janna" ||
                            ...
                            ObjectManager.Player.CharacterName == "Karma")
                        {
                            if (vMiqueiasAIO.Evade.EvadeTargetManager.Menu["whitelist"][
                                        ally.CharacterName.ToLower()] &&
                                aaa.Distance(ObjectManager.Player) < vMiqueiasAIO.Common.Champion.E.Range)
                            {
                                vMiqueiasAIO.Common.Champion.E.CastOnUnit(aaa);
                            }
                        }
                        if (ObjectManager.Player.CharacterName == "Lux" || ObjectManager.Player.CharacterName == "Sona" ||
                            ObjectManager.Player.CharacterName == "Taric")

                        {
                            if (vMiqueiasAIO.Evade.EvadeTargetManager.Menu["whitelist"][
                                        ally.CharacterName.ToLower()] &&
                                aaa.Distance(ObjectManager.Player) < vMiqueiasAIO.Common.Champion.W.Range)
                            {
                                vMiqueiasAIO.Common.Champion.W.CastOnUnit(aaa);
                            }
                        }
                    }
