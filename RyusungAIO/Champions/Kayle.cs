...
namespace RyusungAIO.Champions
{
    class Kayle
    {
        ...
        {
            if (sender is AITurretClient && AutoR["enable"].GetValue<MenuBool>() && sender.Team != Player.Team && args.Target.IsMe)
            {
        ...
        private void OnDoCast(AIBaseClient sender, AIBaseClientProcessSpellCastEventArgs args)
        {
            ...
            if (sender.IsEnemy && AutoR["enable"].GetValue<MenuBool>() && !args.SData.Name.IsAutoAttack())
            {
                if (args.Target != null)
                {
